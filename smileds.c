...
TXDATA_T tx_buffer[TX_BUFF_LEN(CHAN_MAXLEDS)] = {
    0}; // Tx buffer for assembling data
...
bool leds_init(int init_led_count) {
  ...
  printf("smileds: Setting %u LED%s per channel, %u channels\n", led_count,
         led_count == 1 ? "" : "s", LED_NCHANS);
  return true;
}

TXDATA_T *leds_get_buffer() { return tx_buffer; }
...
void leds_set_pixel(uint8_t channel, uint16_t pixel, union color_t color) {
  ...
  if (color.component.a == 255) {
    // no blending, just store (in case there will be another pixel that will be
    // blended on top of this one)
    color_buffer[channel][pixel] = color;
  } else {
    const uint8_t old_a = 1 - color.component.a;
    color.component.r = color_buffer[channel][pixel].component.r * old_a / 255 +
                        color.component.r * color.component.a / 255;
    color.component.g = color_buffer[channel][pixel].component.g * old_a / 255 +
                        color.component.g * color.component.a / 255;
    color.component.b = color_buffer[channel][pixel].component.b * old_a / 255 +
                        color.component.b * color.component.a / 255;
    color_buffer[channel][pixel] = color;
  }
...
void leds_send() {
  ...
  // #endif
  // NOTE: due to caching its more efficient to use a memcpy instead of direct
  // buffer manipulation.
